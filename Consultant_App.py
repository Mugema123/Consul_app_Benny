...
# App Config
st.set_page_config(page_title="Unified AI Consultant", layout="wide")
st.title("🤖 Unified AI Consultant Dashboard")
...
if mode == "🥑 Avocado Market":
    ...
    st.subheader("2️⃣ Inference on New Data")
...
# ---------------- School Dropout Prediction ----------------
    ...
    st.header("🎓 School Dropout Prediction Dashboard")
    ...
    st.subheader("4️⃣ Inference on Uploaded Data")
    uploaded_file = st.file_uploader("📂 Upload labeled CSV (must include `target`)", type=["csv"], key="edu_infer")
    if uploaded_file:
        ...
        try:
            ...
            if st.button("🧠 Consult LLM (Uploaded Data)"):
